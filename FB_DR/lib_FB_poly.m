function [batch,batch_cnt]=lib_FB_poly(param_method, param_solver, y , N, degree, batch, batch_cnt)

% Function which minimazes f1(x) + f2(x) by FB algorithm.
...
beta = param_method.beta;
A = param_method.A_mat;
vector_tau=param_method.tau;
param_tv1d = param_solver.param_tv1d;

% data term
fdata_smooth.eval = @(x) 0.5*norm(A*x-y)^2;
fdata_smooth.grad = @(x) A'*(A*x-y);
fdata_smooth.beta = beta;
...
tic
[sol, info] = forward_backward(start_point, f_nonsmooth, fdata_smooth, param_solver);
time_end_FB=toc;

y_recon = lib_recon_from_params(sol, N, degree, param_method); %synthesize the estimated signal

% save into batch
batch{batch_cnt}.algorithm = 'FB';
batch{batch_cnt}.fdata_smooth = fdata_smooth;
...
batch{batch_cnt}.final_objective = fdata_smooth.eval(sol) + f_nonsmooth.eval(sol);
batch{batch_cnt}.TVx0 = norm(diff(sol(1:N),1));
batch{batch_cnt}.TVx1 = norm(diff(sol(N+1:end),1));
batch{batch_cnt}.elapsed_time = time_end_FB;
